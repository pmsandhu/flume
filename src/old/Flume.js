...
config
  ...
  .addNodeType({
    type: "aws",
    label: "AWS Comprehend",
    description: "Run AWS Comprehend on Value",
    initialWidth: 180,
    inputs: ports => [
      ports.text({ name: "field" }),
      ports.aws({ name: 'fn'})
    ],
  }).addNodeType({
    type: "row",
    ...
    outputs: ports => [
      ...
      ports.number({ name: "age", label: "age" }),
      ports.number({ name: "lat", label: "lat" }),
      ports.number({ name: "lon", label: "lon" }),
      ports.boolean({ name: "married", label: "married" })
    ]
  })
  .addNodeType({
    type: 'geopoint',
    ...
    inputs: ports => [
      ...
      ports.text({ name: 'geofield', label: 'Geopoint Field Name'})
  ...
  .addNodeType({
    ...
    initialWidth: 130,
    outputs: ports => [
      ports.text({ name: "field", label: "field1" }),
